...
impl LeveledCompactionController {
    ...
    pub fn apply_compaction_result(
        ...
        let mut upper_to_remove = task
            .upper_level_sst_ids
            .iter()
            .copied()
            .collect::<HashSet<_>>();
        let mut lower_to_remove = task
            .lower_level_sst_ids
            .iter()
            .copied()
            .collect::<HashSet<_>>();
        if let Some(upper) = task.upper_level {
            let new_upper_ssts = snapshot.levels[upper - 1]
                .1
                .iter()
                .filter_map(|x| {
        ...
        let mut new_lower_ssts = snapshot.levels[task.lower_level - 1]
            .1
            .iter()
            .filter_map(|x| {
